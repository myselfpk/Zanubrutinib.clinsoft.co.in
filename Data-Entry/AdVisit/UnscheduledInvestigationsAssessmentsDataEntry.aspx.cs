...
public partial class Data_Entry_AdVisit_UnscheduledInvestigationsAssessmentsDataEntry : System.Web.UI.Page
{
  ...
  (ConfigurationManager.ConnectionStrings["constr"].ToString());
    ...
    private void PageStatus()
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus,LockStatus from [Add].[UnscheduledInvestigationsAssessments] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt1);
        if (dt1.Rows.Count > 0)
        {
    ...
    private void AEAutoGNumber()
    {
        ...
        con.Open();
        DataTable dt1 = new DataTable();
        SqlCommand cmd = new SqlCommand("SELECT Count(UISNO) as tot FROM [Add].[UnscheduledInvestigationsAssessments] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa1 = new SqlDataAdapter(cmd);
        sqlDa1.Fill(dt1);
        var value = dt1.Rows[0]["tot"].ToString();
        if (value == "0")
        {
            SqlCommand cmd1 = new SqlCommand("SELECT Count(UISNO) as tot FROM [Add].[UnscheduledInvestigationsAssessments] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
            ...
            if (dr1.HasRows)
            {
                while (dr1.Read())
                {
                    ...
                    {
                        int j = i + 1;
                        TextBoxUISNO.Text = j.ToString("000");

                    }
                    else
                    {
                        TextBoxUISNO.Text = "001";
                    }
            ...
            {
                TextBoxUISNO.Text = "000";
            }
            con.Close();
        }
        else
        {
            SqlCommand cmd1 = new SqlCommand("SELECT MAX(UISNO) as tot FROM [Add].[UnscheduledInvestigationsAssessments] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
