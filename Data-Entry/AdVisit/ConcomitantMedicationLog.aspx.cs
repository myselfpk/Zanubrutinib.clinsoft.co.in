...
public partial class Data_Entry_AdVisit_ConcomitantMedicationLog : System.Web.UI.Page
{
...
(ConfigurationManager.ConnectionStrings["constr"].ToString());
    ...
    private void BindPage()
    {
        con.Close();
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("SELECT CNSNO ,CMLMN ,CMLMT ,CMLMTO ,CMLIND ,CMLDS ,CMLUT ,CMLUTO ,CMLFRQ ,CMLFRQO ,CMLRT ,CMLRTO ,CMLSD ,CMLED ,CMLONGO FROM [Add].[ConcomitantMedicationLog] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "' and [CNSNO]='" + Request.QueryString["f"] + "'", con);
    ...
    private void PageStatus()
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus,LockStatus from [Add].[ConcomitantMedicationLog] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "' and [CNSNO]='" + Request.QueryString["f"] + "' and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt1);
        if (dt1.Rows.Count > 0)
        {
    ...
    public void OnConfirm(object sender, EventArgs e)
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus from [Add].[ConcomitantMedicationLog]  where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "' and [CNSNO]='" + Request.QueryString["f"] + "' and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt2);
        if (dt2.Rows.Count > 0)
        {
            ...
            if (dt2.Rows[0]["EntryStatus"].ToString() == "Submit")
            {
                ...
                if (confirmValue == "Yes")
                {
                    ...
                    DataTable dt1 = new DataTable();
                    con.Open();
                    sqlCmd = new SqlCommand("SELECT ActivefLag FROM [Add].[ConcomitantMedicationLog] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "' and [CNSNO]='" + Request.QueryString["f"] + "' and [SUBINI]='" + Request.QueryString["c"] + "' and (ActivefLag='0' or ActivefLag='1')", con);
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        ...
                        {
                            if (ViewState["txt1"].ToString() != TextBoxCNSNO.Text || ViewState["txt2"].ToString() != TextBoxCMLMN.Text || ViewState["txt3"].ToString() != RadioButtonListCMLMT.SelectedValue || ViewState["txt4"].ToString() != TextBoxCMLMTO.Text || ViewState["txt5"].ToString() != TextBoxCMLIND.Text || ViewState["txt6"].ToString() != TextBoxCMLDS.Text || ViewState["txt7"].ToString() != RadioButtonListCMLUT.SelectedValue || ViewState["txt8"].ToString() != TextBoxCMLUTO.Text || ViewState["txt9"].ToString() != RadioButtonListCMLFRQ.SelectedValue || ViewState["txt10"].ToString() != TextBoxCMLFRQO.Text || ViewState["txt11"].ToString() != RadioButtonListCMLRT.SelectedValue || ViewState["txt12"].ToString() != TextBoxCMLRTO.Text || ViewState["txt13"].ToString() != TextBoxCMLSD.Text || ViewState["txt14"].ToString() != TextBoxCMLED.Text || ViewState["txt15"].ToString() != RadioButtonListCMLONGO.SelectedValue)
                            {
