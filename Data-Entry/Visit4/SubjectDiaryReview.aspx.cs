...
public partial class Data_Entry_Visit4_SubjectDiaryReview : System.Web.UI.Page
{
...
(ConfigurationManager.ConnectionStrings["constr"].ToString());
    ...
    private void BindPage()
    {
        con.Close();
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select SDRPER from [Visit4].[SubjectDiaryReview] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
    ...
    private void PageStatus()
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus,LockStatus from [Visit4].[SubjectDiaryReview] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt1);
        if (dt1.Rows.Count > 0)
        {
    ...
    public void OnConfirm(object sender, EventArgs e)
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus from [Visit4].[SubjectDiaryReview] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt2);
        if (dt2.Rows.Count > 0)
        {
            ...
            if (dt2.Rows[0]["EntryStatus"].ToString() == "Save")
            {
                ...
                {
                    con.Close();
                    cmd = new SqlCommand("[Visit4].[sp_SubjectDiaryReview]", con);
            ...
            if (dt2.Rows[0]["EntryStatus"].ToString() == "Submit")
            {
                ...
                if (confirmValue == "Yes")
                {
                    ...
                    DataTable dt1 = new DataTable();
                    con.Open();
                    sqlCmd = new SqlCommand("SELECT ActivefLag FROM [Visit4].[SubjectDiaryReview] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "' and (ActivefLag='0' or ActivefLag='1')", con);
    ...
    protected void ImageQuerySDRPER_Click(object sender, ImageClickEventArgs e)
    {
        con.Close();
        con.Open();
        SqlCommand cmd = new SqlCommand("Select * from [Visit4].[SubjectDiaryReview] where SITENUM='" + lblCenterNumber.Text + "' and SUBNUM = '" + lblScreeningNo.Text + "' and SUBINI = '" + lblSubjectInitial.Text + "'and SDRPER = '" + RadioButtonListSDRPER.SelectedValue + "'", con);
        SqlDataReader dr;
        dr = cmd.ExecuteReader();
        if (dr.Read())
        {
    ...
    protected void QueryRaiseSDRPER_Click(object sender, EventArgs e)
    {
        ...
        cmd1 = new SqlCommand("Update [Visit4].[SubjectDiaryReview] set  QueryStatus = 'Query Responded'  where SITENUM='" + lblCenterNumber.Text + "' and SUBNUM = '" + lblScreeningNo.Text + "' and SUBINI = '" + lblSubjectInitial.Text + "'and SDRPER = '" + RadioButtonListSDRPER.SelectedValue + "'", con1);
