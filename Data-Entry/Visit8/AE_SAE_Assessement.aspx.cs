...
public partial class Data_Entry_Visit8_AE_SAE_Assessement : System.Web.UI.Page
{
...
(ConfigurationManager.ConnectionStrings["constr"].ToString());
    ...
    private void BindPage()
    {
        con.Close();
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select AEASS  from [Visit8].[AE_SAE_Assessement] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
    ...
    private void PageStatus()
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus,LockStatus from [Visit8].[AE_SAE_Assessement] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt1);
        if (dt1.Rows.Count > 0)
        {
    ...
    public void OnConfirm(object sender, EventArgs e)
    {
        ...
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("Select EntryStatus from [Visit8].[AE_SAE_Assessement] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
        SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
        sqlDa.Fill(dt2);
        if (dt2.Rows.Count > 0)
        {
            ...
            if (dt2.Rows[0]["EntryStatus"].ToString() == "Save")
            {
                ...
                {
                    con.Close();
                    cmd = new SqlCommand("[Visit8].[sp_AE_SAE_Assessement]", con);
            ...
            if (dt2.Rows[0]["EntryStatus"].ToString() == "Submit")
            {
                ...
                if (confirmValue == "Yes")
                {
                    ...
                    DataTable dt1 = new DataTable();
                    con.Open();
                    sqlCmd = new SqlCommand("SELECT ActivefLag FROM [Visit8].[AE_SAE_Assessement] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "' and (ActivefLag='0' or ActivefLag='1')", con);
    ...
    protected void ImageQueryAEASS_Click(object sender, ImageClickEventArgs e)
    {
        con.Close();
        con.Open();
        SqlCommand cmd = new SqlCommand("Select * from [Visit8].[AE_SAE_Assessement] where SITENUM='" + lblCenterNumber.Text + "' and SUBNUM = '" + lblScreeningNo.Text + "' and SUBINI = '" + lblSubjectInitial.Text + "'and AEASS = '" + RadioButtonListAEASS.SelectedValue + "'", con);
        SqlDataReader dr;
        dr = cmd.ExecuteReader();
        if (dr.Read())
        {
    ...
    protected void QueryRaiseAEASS_Click(object sender, EventArgs e)
    {
        ...
        cmd1 = new SqlCommand("Update [Visit8].[AE_SAE_Assessement] set  QueryStatus = 'Query Responded'  where SITENUM='" + lblCenterNumber.Text + "' and SUBNUM = '" + lblScreeningNo.Text + "' and SUBINI = '" + lblSubjectInitial.Text + "'and AEASS = '" + RadioButtonListAEASS.SelectedValue + "'", con1);
