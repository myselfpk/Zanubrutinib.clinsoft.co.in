...
public partial class DataView_Visit1_MedicalAndSurgicalHistory : System.Web.UI.Page
{
...
(ConfigurationManager.ConnectionStrings["constr"].ToString());
    ...
    private void BindPage()
    {
        con.Close();
        con.Open();
        SqlCommand sqlCmd = new SqlCommand("SELECT ST1OPT ,DTL1OPT ,ST2OPT ,DTL2OPT ,ST3OPT ,DTL3OPT ,ST4OPT ,DTL4OPT ,MSHYOGMC FROM [Visit1].[MedicalAndSurgicalHistory] where [SITENUM] ='" + Request.QueryString["a"] + "' and [SUBNUM]='" + Request.QueryString["b"] + "'and [SUBINI]='" + Request.QueryString["c"] + "'", con);
    ...
    private void SetImageQueryMSHYOGMC()
    {
        ...
        if (dr.Read())
        {
            if (dr["Status"].ToString() == "Open")
            {
                ...
                ImageQueryMSHYOGMC.ImageUrl = "~/DataView/Images/red.png";


            }
            else if (dr["Status"].ToString() == "Close")
            {
                ImageQueryMSHYOGMC.ImageUrl = "~/DataView/Images/close.png";

            }
            else if (dr["Status"].ToString() == "Query Responded")
            {
                ImageQueryMSHYOGMC.ImageUrl = "~/DataView/Images/Yellow.png";

            }
            else
            {
                ImageQueryMSHYOGMC.ImageUrl = "~/DataView/Images/Blue.jpg";
    ...
    protected void ImageQueryMSHYOGMC_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (dr.Read())
        {
            ...
            if ((ImageQueryMSHYOGMC.ImageUrl == "~/DataView/Images/Blue.jpg") && (dr["LockStatus"].ToString() != "Locked"))
            {
            ...
            else if ((ImageQueryMSHYOGMC.ImageUrl == "~/DataView/Images/red.png") && (dr["LockStatus"].ToString() != "Locked"))
            {
                RAISEMSHYOGMC.Visible = true;
                RAISEMSHYOGMC.ShowPopupWindow();
            }

            else if ((ImageQueryMSHYOGMC.ImageUrl == "~/DataView/Images/Yellow.png") && (dr["LockStatus"].ToString() != "Locked"))
            {
                RespondedMSHYOGMC.Visible = true;
                RespondedMSHYOGMC.ShowPopupWindow();
            }
            else if ((ImageQueryMSHYOGMC.ImageUrl == "~/DataView/Images/close.png") && (dr["LockStatus"].ToString() != "Locked"))
            {
    ...
    protected void QueryRaiseMSHYOGMC_Click(object sender, EventArgs e)
    {
        ...
        cmd = new SqlCommand(" insert into tblQuery values('" + lblCenterNumber.Text + "','" + lblScreeningNo.Text + "','" + lblSubjectInitial.Text + "','" + lblVisit.Text + "','" + lblPage.Text + "','" + lblMSHYOGMC.Text + "','" + LabelUserName.Text + "','" + DateTime.Today.ToString("dd-MMM-yyyy") + "','','','','Open','','','','','')", con);
